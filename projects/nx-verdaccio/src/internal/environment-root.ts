...
export function getEnvironmentDir(context: ExecutorContext) {
  ...
  return plugin.options.environments?.environmentsDir ?? DEFAULT_ENVIRONMENTS_OUTPUT_DIR;
}

export function getEnvironmentRoot(context: ExecutorContext, options: Partial<Environment>) {
