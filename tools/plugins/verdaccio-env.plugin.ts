...
export const createNodes: CreateNodes = [
  ...
  (
    ...
    const isPublishable = (projectConfiguration?.tags ?? []).some(target => target === 'publishable');
    if (!isPublishable) {
      return {};
    }
...
function npmTargets(projectConfiguration: ProjectConfiguration) {
  ...
  const {name: packageName, version: pkgVersion} = readJsonFile(join(root, 'package.json'));
  return {
    'npm-install': {
